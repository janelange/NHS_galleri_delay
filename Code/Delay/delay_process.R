#process data for trial simulations
library(dplyr)
...
advance_dx_time_stage<-function(the_data,delay,early_to_late_rate){
  ...
  the_data<-the_data %>% mutate(clinical_diagnosis_time=clinical_diagnosis_time+delay,clinical_diagnosis_stage=new_stage)%>%
...
results=advance_dx_time_stage(the_data=results,delay=the_delay,early_to_late=2)

#Save summary data file
save_file_name=file.path(delay_dirname,paste0("delay_", i,".Rdata"))
