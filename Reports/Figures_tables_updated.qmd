...
counts_plot <- ggdraw(plot_with_top) +
  ...
  plot_rel_reduction <- ggplot(subset(counts_long,
    ...
    theme_mced() +
    coord_cartesian(xlim=c(0,90),ylim=c(-15,25))+
    labs(x = "Diagnosis delay (days)", y = "Relative reduction (%)")
  ...
  plot_pct_bias_rel_reduction <- ggplot(subset(results_bias,variable=="rel_reduction"), aes(delay, pct_bias)) +
    geom_line(linewidth = 1,aes(color=scenario_no,group=scenario_no)) +
    geom_point(size = 2,aes(color=scenario_no)) +
    coord_cartesian(xlim=c(0,90),ylim=c(-350,15)) +scale_color_manual(values = pal[4:10])+
    # geom_errorbar(aes(ymin=mean-se,ymax=mean+se),alpha=.18)+
    theme_mced() +
    labs(x = "Diagnosis delay (days)", y = "Percent Bias in trial outcome")
  
  plot_abs_bias_rel_reduction <- ggplot(subset(results_bias,variable=="rel_reduction"), aes(delay, abs_bias)) +
    geom_line(linewidth = 1,aes(color=scenario_no,group=scenario_no)) +
    geom_point(size = 2,aes(color=scenario_no)) +
    coord_cartesian(xlim=c(0,90),ylim=c(-30,5)) +scale_color_manual(values = pal[4:10])+
    # geom_errorbar(aes(ymin=mean-se,ymax=mean+se),alpha=.18)+
    theme_mced() +
...
plots_scen_C <- make_delay_plots2(extended_followup=0,the_early_to_late_rate=0,screen_arm_delay=1,the_scenario=1)
plots_scen_D <- make_delay_plots2(extended_followup=0,the_early_to_late_rate=0,screen_arm_delay=1,the_scenario=2)
plots_scen_E <- make_delay_plots2(extended_followup=0,the_early_to_late_rate=0,screen_arm_delay=1,the_scenario=3)

df=bind_rows(plots_scen_C$counts_only,plots_scen_D$counts_only,plots_scen_E$counts_only)
...
tab <- df %>%
  ...
  mutate(
    extended_followup = if_else(extended_followup == 1, "Yes", "No"),
  ...
  arrange(scenario_no, extended_followup, arms_delay_affects, delay) %>%
  select(
    `NH model` = scenario_no,
...
make_scenario_latex <- function(tab, scenario,scenario_label) {
 ...
 print(make_scenario_latex(tab, 1,scenario_label="C"))
 print(make_scenario_latex(tab, 2,scenario_label="D"))
 print(make_scenario_latex(tab, 3,scenario_label="E"))
```
...
## Figure: Observed trial endpoints and absolute and relative bias given diagnostic delay (continued follow-up; clinical cancers only)--end of trial and extended followup combined
```{r}
